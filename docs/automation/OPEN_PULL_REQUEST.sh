...
# Optional runtime vars provided by caller
BASE_BRANCH_ARG="${1:-}"          
...
if ! git rev-parse --abbrev-ref "@{u}" >/dev/null 2>&1; then
  fail "Current branch has no upstream. Push the branch first (use PUSH_BRANCH.sh) before opening a PR."
fi
...
According to Docs-as-System mini policy, this PR requires human review before any merge."
fi
